...
public class ImageMain implements ActionListener {
	 ...
	 */
	private static final String[] IMAGE_ALGORITHMS = { "Load Image", "Save Image", "Diff Images",
			"Grayscale", "Negative", "Rotate Left", "Rotate Right", "Translate", "Blur", "Mystery" };
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd == "Load Image") {
			...
			canvas.setImage(imageAlgorithms.grayscale(canvas.getImage()));
			statusLabel.setText("Grayscale filter applied.");
		} else if (cmd == "Negative") {
			canvas.setImage(imageAlgorithms.negative(canvas.getImage()));
			statusLabel.setText("Negative filter applied.");
		} else if (cmd == "Rotate Left") {
			canvas.setImage(imageAlgorithms.rotateLeft(canvas.getImage()));
			statusLabel.setText("Rotate left filter applied.");
		} else if (cmd == "Rotate Right") {
			canvas.setImage(imageAlgorithms.rotateRight(canvas.getImage()));
			statusLabel.setText("Rotate right filter applied.");
		} else if (cmd == "Translate") {
			int dx = readInteger("dx?");
			int dy = readInteger("dy?");
			canvas.setImage(imageAlgorithms.translate(canvas.getImage(), dx, dy));
			statusLabel.setText("Translate filter applied.");
		} else if (cmd == "Blur") {
			canvas.setImage(imageAlgorithms.blur(canvas.getImage()));
			statusLabel.setText("Blur filter applied.");
		} else if (cmd == "Mystery") {
			canvas.setImage(imageAlgorithms.colorMashup(canvas.getImage()));
	...
	private void doLayout() {
		...
		for (String algorithm : getImageAlgorithms()) {
			...
			bar.add(GuiUtils.createButton(algorithm, iconFolder + icon, mnemonic, this));
		}
	...
	private void runMysteryAlgorithm(String name) {
		...
		try {
			...
			GImage result = (GImage) mysteryMethod.invoke(imageAlgorithms, source);
